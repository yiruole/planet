...
WOODS = [worn_mat('wood_%d' % i, c, 0.62, 0.45)
...
def chair():
    ...
    P_box((0.05, 0.44, 0.5), (-0.22, 0, 0.72))                    # back left rail? -> frame:
...
for i in range(NP):
    ...
    for attempt in range(60):
        u = random.random()
        r = 1.95 * (u ** 0.5)            # center-dense -> mound
        ...
        limit = 3.0 * max(0.0, 1.0 - r / 3.3)   # pile must slope down outward
        if support <= limit:
            ok = True
            break
    if not ok:
...
all_off()
for L, k in fluos: L.data.energy = 60 * k
for i, (pm, k) in enumerate(panel_mats): set_emit(pm, 2.2 * k)
...
bpy.ops.wm.save_as_mainfile(filepath=OUT + '/backrooms_v2.blend')
